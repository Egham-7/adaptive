...
**Note**: Semantic similarity threshold is managed automatically by the system for optimal performance. The prompt cache uses the same semantic matching technology as the model router's cache.
