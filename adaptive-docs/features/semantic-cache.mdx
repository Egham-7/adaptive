...
// Customize threshold for stricter matching
{
  ...
  "model_router": {
    "semantic_cache": {
      "threshold": 0.9  // Higher = stricter matching
    }
  }
}
...
- `threshold`: Float between 0.0-1.0 for similarity matching (default: 0.85)
...
Responses include `cache_tier` to show the cache type used:

- `"semantic_exact"` - Exact match found
- `"semantic_similar"` - Similar match found
...
- **Storage**: Uses model router's AI service for embedding generation and matching
