---
title: 'Select Model'
api: 'POST https://llmadaptive.com/api/v1/select-model'
...
<ParamField body="models" type="array" required>
  ...
  <Expandable title="Model Specification Options">
    ...
    **Full specification (for custom models):**
    ...
    - **supports_function_calling** (boolean): Whether the model supports function/tool calling (auto-filled for known models)
...
response=$(curl -s -w "\n%{http_code}" https://llmadaptive.com/api/v1/select-model \
...
const res = await fetch('https://llmadaptive.com/api/v1/select-model', {
  method: 'POST',
...
const shouldUseCheap = await fetch('https://llmadaptive.com/api/v1/select-model', {
  method: 'POST',
  headers: { 'X-Stainless-API-Key': adaptiveKey, 'Content-Type': 'application/json' },
  body: JSON.stringify({
...
const shouldUseExpensive = await fetch('https://llmadaptive.com/api/v1/select-model', {
  method: 'POST',
  headers: { 'X-Stainless-API-Key': adaptiveKey, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    models: [{ provider: "openai", model_name: "gpt-4o-mini" }, { provider: "openai", model_name: "gpt-4o" }],
