...
<AccordionGroup>
  <Accordion title="Why am I not getting responses from certain providers?">
    Check your `protocol_manager.models` configuration. Ensure the providers you want are included and your `cost_bias` setting allows for the provider selection you expect.
  </Accordion>
  ...
  <Accordion title="Can I force a specific model?">
    Use the `protocol_manager.models` array with specific model names:
    ```javascript
    protocol_manager: {
      models: [
