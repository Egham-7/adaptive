...
<CodeGroup>
```bash npm
npm install ai @adaptive-llm/adaptive
```

```bash yarn
yarn add ai @adaptive-llm/adaptive
```

```bash pnpm
pnpm add ai @adaptive-llm/adaptive
```
</CodeGroup>
...
```bash yarn
yarn add @adaptive-llm/adaptive
```

```bash pnpm
pnpm add @adaptive-llm/adaptive
```
</CodeGroup>
...
Control routing behavior with detailed configuration options using the Adaptive provider:
...
await generateText({
  model: adaptiveOpenAI(''),
  prompt: "Summarize this article",
  // Note: Advanced configuration options are available 
  // through request headers or model selection
});
```
</CodeGroup>
...
```javascript API Route (Next.js) - Adaptive Provider
// pages/api/chat.js or app/api/chat/route.js
import { adaptive } from '@adaptive-llm/adaptive';
import { streamText } from 'ai';
...
```javascript API Route (Next.js) - OpenAI Provider
// pages/api/chat.js or app/api/chat/route.js
import { openai } from '@ai-sdk/openai';
import { streamText } from 'ai';
