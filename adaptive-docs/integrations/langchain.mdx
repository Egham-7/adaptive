...
const llm = new ChatOpenAI({
  apiKey: "ADAPTIVE_KEY",
  configuration: { baseURL: "https://llmadaptive.uk/api/v1" },
  model: "",
});
...
```python
llm = ChatOpenAI(..., model_kwargs={"provider_constraints":["openai","anthropic"], "cost_bias":0.2})
```
...
| **Add `provider_constraints`** | Controls which API providers are used |
