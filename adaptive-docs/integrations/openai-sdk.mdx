...
Use custom providers alongside standard ones:
...
const completion = await openai.chat.completions.create({
  ...
  // Intelligent routing with custom provider
  model_router: {
    models: [
...
You can also override configuration for standard providers like OpenAI, Anthropic, etc.:

```javascript
// Use custom OpenAI endpoint or proxy
const completion = await openai.chat.completions.create({
  model: '',
  messages: [
    { role: 'user', content: 'Hello from my custom endpoint!' }
  ],
  model_router: {
    models: [
  ...
  // Override standard provider settings
  provider_configs: {
    "openai": {
