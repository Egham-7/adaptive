...
**Automated installer** - Run this one-line command to automatically configure OpenCode:
...
The installer will:
- Install Node.js and OpenCode if needed
...
**Manual setup** - Follow the detailed steps below if you prefer manual configuration.
...
```bash
opencode auth login
# Select "Other" → Enter "adaptive" → Paste API key
```
...
```bash Reinstall with Script
# Remove existing config and reinstall
rm -f opencode.json
curl -fsSL https://raw.githubusercontent.com/Egham-7/adaptive/main/scripts/installers/opencode.sh | bash
```
</CodeGroup>
