...
export class CreditService {
	...
	static async getOrganizationBalance(organizationId: string): Promise<number> {
		try {
			const orgCredit =
				await CreditService.getOrCreateOrganizationCredit(organizationId);
	...
	static async hasSufficientCredits(
		...
		const balance = await CreditService.getOrganizationBalance(organizationId);
	...
	static calculateCreditCost(
		...
		const outputCost =
			(outputTokens / 1_000_000) * OUTPUT_TOKEN_COST_PER_MILLION;
