    ...
    >
      <ReactMarkdown
        ...
        components={{
          ...
          pre: ({ children }) => (
            <pre className="overflow-auto w-full rounded-md">{children}</pre>
          ),
          ul: ({ children }) => (
          ...
          blockquote: ({ children }) => (
            <blockquote className="pl-4 border-l-4 border-muted-foreground/30 italic text-muted-foreground">
              {children}
            </blockquote>
          ),
          h1: ({ children }) => (
            <h1 className="text-2xl font-bold mt-6 mb-4 break-words">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-xl font-bold mt-6 mb-3 break-words">{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-lg font-bold mt-5 mb-2 break-words">{children}</h3>
          ),
          h4: ({ children }) => (
            <h4 className="text-base font-bold mt-4 mb-2 break-words">{children}</h4>
          ),
          h5: ({ children }) => (
            <h5 className="text-sm font-bold mt-4 mb-1 break-words">{children}</h5>
          ),
          h6: ({ children }) => (
            <h6 className="text-xs font-bold mt-4 mb-1 break-words">{children}</h6>
          ),
        }}
      >
        {content}
