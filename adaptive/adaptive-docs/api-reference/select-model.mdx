...
response=$(curl -s -w "\n%{http_code}" https://www.llmadaptive.uk/api/v1/select-model \
...
const res = await fetch('https://www.llmadaptive.uk/api/v1/select-model', {
  method: 'POST',
...
const shouldUseCheap = await fetch('https://www.llmadaptive.uk/api/v1/select-model', {
  method: 'POST',
  headers: { 'X-Stainless-API-Key': adaptiveKey, 'Content-Type': 'application/json' },
  body: JSON.stringify({
...
const shouldUseExpensive = await fetch('https://www.llmadaptive.uk/api/v1/select-model', {
  method: 'POST',
  headers: { 'X-Stainless-API-Key': adaptiveKey, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    models: [{ provider: "openai", model_name: "gpt-4o-mini" }, { provider: "openai", model_name: "gpt-4o" }],
