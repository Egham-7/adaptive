...
class ProtocolTester:
    ...
    def test_model_selection(
        ...
        if not self.client.health_check():
            raise ConnectionError(
                f"adaptive_ai service is not available at {self.service_url}"
            )
        ...
        for i, conversation in enumerate(conversations):
            try:
                ...
                local_task_type, _ = self.model_selector.analyze_conversation(
                    conversation
                )

                # Make request to adaptive_ai service
                service_response: AdaptiveAIResponse = self.client.make_request(
                    user_prompt
                )
    ...
    def analyze_results(self) -> dict[str, Any]:
        ...
        for result in self.results:
            if result.success:
                model_usage[result.selected_model] = (
                    model_usage.get(result.selected_model, 0) + 1
                )
                protocol_usage[result.protocol] = (
                    protocol_usage.get(result.protocol, 0) + 1
                )
                task_distribution[result.task_type.value] = (
                    task_distribution.get(result.task_type.value, 0) + 1
                )
...
"""
        ...
        for model, count in sorted(
            analysis["model_usage"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for protocol, count in sorted(
            analysis["protocol_usage"].items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (count / analysis["successful_tests"]) * 100
        ...
        for task, count in sorted(
            analysis["task_distribution"].items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (count / analysis["successful_tests"]) * 100
