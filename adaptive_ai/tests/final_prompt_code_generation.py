...
ds = load_dataset("Usaid/Final_Prompt_Dataset")
# Select only Prompt and Difficulty columns
filtered_ds = ds['train'].select_columns(['Question', 'Difficulty'])

# Count difficulty levels
difficulty_counts = Counter(filtered_ds['Difficulty'])
...
for difficulty in ['Easy', 'Hard']:
    count = len(filtered_ds.filter(lambda x: x['Difficulty'] == difficulty))
    print(f"{difficulty}: {count}")

# Get examples for each difficulty level
easy_examples = filtered_ds.filter(lambda x: x['Difficulty'] == 'Easy')
hard_examples = filtered_ds.filter(lambda x: x['Difficulty'] == 'Hard')
...
def process_difficulty_level(examples, difficulty: str) -> Dict[str, Any]:
    ...
    print(f"\nAnalyzing API responses for {difficulty.lower()} questions...")
    for i, example in enumerate(examples):
        if i % 100 == 0:  # Progress indicator
            print(f"Processing question {i}/{len(examples)}")
        
        request_data = {"prompt": example["Question"]}
        try:
...
md_content = ["# Vector Analysis Report - Easy vs Hard Comparison\n"]
...
for comp in components:
    ...
    md_content.append(f"| {comp} | {easy_mean:.3f} | {hard_mean:.3f} | {diff:+.3f} | {pct_diff:+.1f}% |")
...
for comp in components:
    easy_range = difficulty_stats['easy'][comp]['max'] - difficulty_stats['easy'][comp]['min']
    hard_range = difficulty_stats['hard'][comp]['max'] - difficulty_stats['hard'][comp]['min']
    diff = hard_range - easy_range
    md_content.append(f"| {comp} | {easy_range:.3f} | {hard_range:.3f} | {diff:+.3f} |")

# 4. Summary Statistics
md_content.append("\n## 4. Summary Statistics\n")
md_content.append("| Difficulty | Total Questions | Successful | Failed | Success Rate |")
md_content.append("|------------|----------------|------------|--------|--------------|")

for difficulty, data in results.items():
    total = len(data['vectors']) + len(data['failed_questions'])
    success_rate = len(data['vectors']) / total * 100 if total > 0 else 0
    md_content.append(f"| {difficulty.capitalize()} | {total} | {len(data['vectors'])} | "
...
md_content.append("- Percentage differences show relative change between difficulty levels\n")

md_content.append("\n### Distribution Differences\n")
md_content.append("- Larger standard deviation in Hard questions indicates more variability\n")
md_content.append("- Range differences show the spread of values in each difficulty level\n")

# Save to file
with open('vector_analysis_report_easy_vs_hard.md', 'w') as f:
    f.write('\n'.join(md_content))

print("\nAnalysis report has been saved to 'vector_analysis_report_easy_vs_hard.md'")