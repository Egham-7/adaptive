...
try:
    ...
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'complexity_distribution.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'complexity_boxplot.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for task_type, count in results['task_type_counts'].items():
        task_scores = [score for i, score in enumerate(results['complexity_scores']) 
    ...
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'complexity_by_task_type.png', dpi=300, bbox_inches='tight')
    plt.close()
...
md_content.append(f"| {total} | {len(results['complexity_scores'])} | {len(results['failed_questions'])} | {success_rate:.1f}% |")

# 5. Key Findings
md_content.append("\n## 5. Key Findings\n")
md_content.append("### Complexity Score Analysis\n")
md_content.append("- Mean complexity score shows the average difficulty level across all questions\n")
...
md_content.append("![Complexity Distribution](visualizations/complexity_distribution.png)\n")
md_content.append("This plot shows the distribution of complexity scores, helping identify patterns and outliers. The KDE (Kernel Density Estimation) curve shows the probability density of the scores.\n")

md_content.append("\n### Complexity Score Box Plot\n")
md_content.append("![Complexity Box Plot](visualizations/complexity_boxplot.png)\n")
md_content.append("Box plot provides a clear view of the statistical distribution of complexity scores, including median (center line), quartiles (box edges), and outliers (points beyond whiskers).\n")
...
md_content.append("![Complexity by Task Type](visualizations/complexity_by_task_type.png)\n")
md_content.append("This box plot shows how complexity scores vary across different task types, helping identify which types of tasks tend to be more complex.\n")
