...
try:
    ...
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'component_distributions.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'component_boxplots.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'correlation_heatmap.png', dpi=300, bbox_inches='tight')
    plt.close()

    # 4. Task type distribution pie chart
    plt.figure(figsize=(10, 8))
    plt.pie(results['task_type_counts'].values(), 
            labels=results['task_type_counts'].keys(),
            autopct='%1.1f%%',
            textprops={'fontsize': 10})
    plt.title('Task Type Distribution')
    plt.tight_layout()
    plt.savefig(visualizations_dir / 'task_type_distribution.png', dpi=300, bbox_inches='tight')
    plt.close()

    # 5. Additional: Scatter plot matrix for components
    plt.figure(figsize=(15, 15))
    sns.pairplot(pd.DataFrame(vectors_array, columns=components))
    plt.savefig(visualizations_dir / 'component_scatter_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()
...
md_content.append("![Component Distributions](visualizations/component_distributions.png)\n")
md_content.append("This plot shows the distribution of values for each component, helping identify patterns and outliers. The KDE (Kernel Density Estimation) curve shows the probability density of the values.\n")

md_content.append("\n### Component Box Plots\n")
md_content.append("![Component Box Plots](visualizations/component_boxplots.png)\n")
md_content.append("Box plots provide a clear view of the statistical distribution of each component, including median (center line), quartiles (box edges), and outliers (points beyond whiskers).\n")

md_content.append("\n### Component Correlations\n")
md_content.append("![Component Correlations](visualizations/correlation_heatmap.png)\n")
md_content.append("This heatmap shows the correlation between different components, helping identify relationships between dimensions. Values range from -1 (perfect negative correlation) to 1 (perfect positive correlation).\n")

md_content.append("\n### Task Type Distribution\n")
md_content.append("![Task Type Distribution](visualizations/task_type_distribution.png)\n")
md_content.append("Pie chart showing the distribution of different task types in the dataset, with percentage labels for each category.\n")

md_content.append("\n### Component Scatter Matrix\n")
md_content.append("![Component Scatter Matrix](visualizations/component_scatter_matrix.png)\n")
md_content.append("This scatter plot matrix shows the relationships between all pairs of components, with histograms on the diagonal showing individual distributions.\n")
