...
class CostAnalysisPipeline:
    ...
    def process_predictions(
        ...
        if self.token_counts_df is not None:
            for _, row in tqdm(
                self.token_counts_df.iterrows(), total=len(self.token_counts_df)
            ):
            ...
            if self.token_counts_df is not None:
                ...
                self.combined_df["output_tokens"] = self.token_counts_df[
                    "output_tokens"
                ]
                self.combined_df["total_tokens"] = (
                    self.combined_df["input_tokens"] + self.combined_df["output_tokens"]
                )
                self.combined_df.to_csv(
                    "./prediction_results_with_tokens.csv", index=False
                )
    ...
    def calculate_costs(self) -> None:
        ...
        def calculate_cost(row: pd.Series) -> float:
            ...
            if model not in TOKEN_PRICES:
                return 0.0
            input_cost = (float(row["input_tokens"]) / 1_000_000) * TOKEN_PRICES[model][
                "input"
            ]
            output_cost = (float(row["output_tokens"]) / 1_000_000) * TOKEN_PRICES[
                model
            ]["output"]
    ...
    def calculate_single_model_cost(self, model_name: str) -> float:
        ...
        if model_name not in TOKEN_PRICES or self.costs_df is None:
            return 0.0

        total_input_tokens = float(self.costs_df["input_tokens"].sum())
        total_output_tokens = float(self.costs_df["output_tokens"].sum())

        input_cost = (total_input_tokens / 1_000_000) * TOKEN_PRICES[model_name][
            "input"
        ]
        output_cost = (total_output_tokens / 1_000_000) * TOKEN_PRICES[model_name][
            "output"
        ]
        return input_cost + output_cost
    ...
    def generate_visualizations(self) -> None:
        ...
        ax1.axhline(
            y=adaptive_cost, color="red", linestyle="--", label="Adaptive Model Cost"
        )
        ...
        task_costs = (
            self.costs_df.groupby("task_type")["cost"]
            .sum()
        ...
        model_token_costs = (
            self.costs_df.groupby("model")
            .agg({"cost": "sum", "total_tokens": "sum"})
            .reset_index()
        )
        model_token_costs["cost_per_token"] = (
            model_token_costs["cost"] / model_token_costs["total_tokens"] * 1000
        )
        model_token_costs = model_token_costs.sort_values(
            "cost_per_token", ascending=False
        )

        bars3 = ax3.bar(
            model_token_costs["model"],
            model_token_costs["cost_per_token"],
            color="salmon",
            alpha=0.7,
        )
    ...
    def print_analysis(self) -> None:
        ...
        provider_costs = (
            self.costs_df.groupby("provider")
            .agg(
                {
                    "cost": ["sum", "mean", "count"],
                    "input_tokens": "sum",
        ...
        task_costs = (
            self.costs_df.groupby("task_type")
            .agg(
                {
                    "cost": ["sum", "mean", "count"],
                    "input_tokens": "sum",
