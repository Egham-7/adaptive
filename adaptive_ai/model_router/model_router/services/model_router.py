...
from model_router.services.model_registry import ModelRegistry
...
class ModelRouter:
    ...
    def select_models(
        ...
        Args:
            ...
            models_input: List of ModelCapability objects (None = use all available)
            cost_bias: Cost vs. capability preference (0.0=cheap, 1.0=capable)
    ...
    def _get_candidate_models(
        ...
        for model in models_input:
            if model.is_partial:
                # Use as filter criteria
                matching = self.model_registry.find_models_matching_criteria(model)
                self._validate_resolved_models(matching)
                candidates.extend(matching)
            else:
                # Use directly or get from registry
    ...
    def _get_all_models_for_task(self, task_type: str) -> list[ModelCapability]:
        ...
        all_names = self.model_registry.get_all_model_names()
        models = []
        for name in all_names:
            model = self.model_registry.get_model_capability(name)
            if model:
                models.append(model)
