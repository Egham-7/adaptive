...
class HeliconeUpdater:
    ...
    def fetch_provider_models(self, provider: str) -> list[dict[str, Any]]:
    ...
    def convert_to_structured_format(
        self, provider: str, raw_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        for model in raw_data:
            ...
            if model.get("cache_read_cost_per_1m"):
                additional_pricing["cache_read_per_1m"] = model[
                    "cache_read_cost_per_1m"
                ]
            if model.get("cache_write_cost_per_1m"):
                additional_pricing["cache_write_per_1m"] = model[
    ...
    def create_provider_yaml(self, provider: str, models: dict[str, Any]) -> Path:
        ...
        with open(yaml_file, "w", encoding="utf-8") as f:
            ...
            f.write("# Source: Helicone API (https://helicone.ai/api/llm-costs)\n")
    ...
    def merge_with_existing(
        self, yaml_file: Path, new_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def update_master_index(self, updated_providers: list[str]) -> None:
        ...
        for yaml_file in self.data_dir.glob("*_models_structured.yaml"):
            ...
            try:
                ...
                providers_info[provider_name] = {
                    ...
                    "sample_models": (
                        list(models.keys())[:3] + [f"... and {len(models)-3} more"]
                        if len(models) > 3
        ...
        with open(index_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "# Each provider has its own structured YAML file with complete pricing\n"
            )
            f.write("# \n")
            f.write("# 📊 Database Statistics:\n")
            f.write(f"#   • Total Providers: {len(providers_info)}\n")
            f.write(f"#   • Total Models: {total_models}\n")
            f.write("#   • Data Source: Live Helicone API\n")
    ...
    def update_providers(
        self, providers: list[str] | None = None, dry_run: bool = False
    ) -> None:
        ...
        logger.info("🚀 Starting Helicone database update...")
        ...
        for provider in providers:
            try:
                ...
                structured_models = self.convert_to_structured_format(
                    provider, raw_data
                )
        ...
        # Summary
        total_models = sum(
            len(yaml.safe_load(f.open())["models"])
