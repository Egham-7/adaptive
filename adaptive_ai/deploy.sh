...
CONTAINER_APP_NAME="backend-python"
...
if ! docker buildx inspect --bootstrap; then
  log "ERROR: Failed to set up Docker Buildx."
  exit 1
fi
...
# Build and push Docker image using buildx for proper platform support
...
if ! az containerapp update \
  ...
  --set-env-vars "ENVIRONMENT=production"; then
